...
docker build $DOCKER_ROOT/llava3d -t llava_3d
docker build $DOCKER_ROOT/vicuna -t -t vicuna_v1.1_7b -t vicuna_v1.1_13b
docker build $DOCKER_ROOT/phi3_vision -t phi_vision
docker build $DOCKER_ROOT/others -t qwen_vl -t llava -t llama_3 -t phi_3.5_mini -t llama_mesh
